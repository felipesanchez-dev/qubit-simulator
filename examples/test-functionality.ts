...
test(
  "Qubit |0⟩ tiene probabilidad 1 para |0⟩",
  Math.abs(qubit0.getProbabilityZero() - 1) < 1e-10
);
test(
  "Qubit |0⟩ tiene probabilidad 0 para |1⟩",
  Math.abs(qubit0.getProbabilityOne() - 0) < 1e-10
);

// Test 2: Crear qubit en estado |1⟩
const qubit1 = Qubit.one();
test(
  "Qubit |1⟩ tiene probabilidad 0 para |0⟩",
  Math.abs(qubit1.getProbabilityZero() - 0) < 1e-10
);
test(
  "Qubit |1⟩ tiene probabilidad 1 para |1⟩",
  Math.abs(qubit1.getProbabilityOne() - 1) < 1e-10
);
...
test(
  "Hadamard en |0⟩ da 50% probabilidad para |0⟩",
  Math.abs(qubitH.getProbabilityZero() - 0.5) < 1e-10
);
test(
  "Hadamard en |0⟩ da 50% probabilidad para |1⟩",
  Math.abs(qubitH.getProbabilityOne() - 0.5) < 1e-10
);
test(
  "Qubit después de Hadamard está en superposición",
  qubitH.isInSuperposition()
);
...
test(
  "Circuito 2-qubit inicia en |00⟩",
  Math.abs(circuit.getMeasurementProbabilities()[0] - 1) < 1e-10
);
...
const totalProb =
  customQubit.getProbabilityZero() + customQubit.getProbabilityOne();
...
if (passedTests === totalTests) {
  console.log(
    `🎉 ¡Todos los tests pasaron! La librería QKits funciona correctamente.`
  );
} else {
