...
for (const [state, count] of bellResults.entries()) {
  console.log(
    `|${state}⟩: ${count} veces (${((count / 1000) * 100).toFixed(1)}%)`
  );
}
...
console.log(
  `Estado |+⟩ - P(|0⟩): ${plusProbs[0].toFixed(
    3
  )}, P(|1⟩): ${plusProbs[1].toFixed(3)}`
);

// Verificar que es aproximadamente 0.5, 0.5
const isCorrectPlus =
  Math.abs(plusProbs[0] - 0.5) < 0.001 && Math.abs(plusProbs[1] - 0.5) < 0.001;
