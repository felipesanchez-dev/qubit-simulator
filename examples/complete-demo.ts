...
console.log(`   Estado inicial: P(|0⟩)=${qubit.getProbabilityZero().toFixed(3)}, P(|1⟩)=${qubit.getProbabilityOne().toFixed(3)}`);

// Aplicar Hadamard
const H = Gates.H();
qubit.setState(H.apply(qubit.getState()));
console.log(`   Después de H: P(|0⟩)=${qubit.getProbabilityZero().toFixed(3)}, P(|1⟩)=${qubit.getProbabilityOne().toFixed(3)}`);
...
console.log(`   Probabilidades: |00⟩=${bellProbs[0].toFixed(3)}, |01⟩=${bellProbs[1].toFixed(3)}, |10⟩=${bellProbs[2].toFixed(3)}, |11⟩=${bellProbs[3].toFixed(3)}`);
...
console.log(`   Suma de probabilidades: ${totalProbability.toFixed(10)} (debe ser 1.0)`);
console.log(`   Conservación: ${Math.abs(totalProbability - 1) < 1e-10 ? '✅' : '❌'}\n`);
...
console.log(`   |00⟩: ${measurements['00']} veces (${(measurements['00']/10).toFixed(1)}%)`);
console.log(`   |11⟩: ${measurements['11']} veces (${(measurements['11']/10).toFixed(1)}%)`);
